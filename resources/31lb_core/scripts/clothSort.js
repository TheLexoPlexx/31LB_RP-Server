...
const filePath = "./src/31lb_core/data_dumps/pedComponentVariations";
const filePathExport = "./src/31lb_core/client/shops/inventories/clothing";
...
export function sortClothes() {
    let clothFile = JSON.parse(fs.readFileSync(filePath + ".json", "utf-8"));
    let tempCloth = {
        cHash: null,
        price: 0,
        drawable: -1,
    ...
    fs.writeFile(filePathExport + ".ts", "let obj = " + JSON.stringify(customClothCache, null, 2) + ";", "utf-8", (err) => {
        if (err != null) {
            console.log(err);
        }
    });
